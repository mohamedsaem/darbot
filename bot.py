...
BOT_TITLE = os.getenv('BOT_TITLE', 'Company Archive Bot')
...
def unique_companies(df: pd.DataFrame) -> List[str]:
    companies = sorted({
        c.strip() for c in df.loc[(df['Top_Section'] == 'أوامر الصرف') & (df['Company'] != ''), 'Company'].tolist() if c.strip()
    })
    return companies
...
def search_order_folders(df: pd.DataFrame, company: str, order_number: str) -> pd.DataFrame:
    rows = df[
        ...
        & (df['Company'] == company)
        & (df['Name'].astype(str).str.strip() == order_number.strip())
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith('company:'):
        ...
        await reply_or_edit(update, f'تم اختيار شركة <b>{company}</b>\nابعت الآن رقم أمر الصرف.', InlineKeyboardMarkup([[InlineKeyboardButton('🏠 الرئيسية', callback_data='home')]]))
        return
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == STATE_AWAIT_ORDER_NUMBER:
        ...
        if not company:
            context.user_data['state'] = STATE_IDLE
            await update.message.reply_text('اختار الشركة أولاً.', reply_markup=main_menu(), parse_mode=ParseMode.HTML)
            return
        df = await get_df(context)
    ...
    await update.message.reply_text('اضغط /start أو اختار من القايمة.', reply_markup=main_menu(), parse_mode=ParseMode.HTML)
